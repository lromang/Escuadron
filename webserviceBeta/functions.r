...
#########################################
get_data_inst <- function(inst){
    query <- paste0("http://catalogo.datos.gob.mx/api/3/action/package_search?q=",inst)
    data  <- RJSONIO::fromJSON(getURL(query))$result$results
    name  <- laply(data, function(t)t <- t$title)
    note  <- laply(data, function(t)t <- t$notes)
    #url   <- laply(data, function(t)t <- t$url)
    data.frame(inst = rep(inst,length(name)), name = name, desc = note)
}

get_inst_desc <- function(inst){
    query <- paste0("http://catalogo.datos.gob.mx/api/3/action/package_search?q=",inst)
    data  <- RJSONIO::fromJSON(getURL(query))$result$results
    data[[1]]$organization$description
}
